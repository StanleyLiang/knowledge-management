 ...
 *
 * @param jobId - The conversion job ID
 * @param onStatus - Callback for each status update
 * @param natsWsUrl - NATS WebSocket URL (default: ws://localhost:9222)
 * @param subjectPrefix - Subject prefix (default: video.convert.status)
 */
export function subscribeToJobStatus(
  jobId: string,
  onStatus: (status: VideoJobStatus) => void,
  natsWsUrl: string = 'ws://localhost:9222',
  subjectPrefix: string = 'video.convert.status',
): () => void {
  const subject = `${subjectPrefix}.${jobId}`
